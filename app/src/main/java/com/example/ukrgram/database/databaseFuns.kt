package com.example.ukrgram.database
...
fun createGroupToDatabase(
    ...
    val keyGroup = REF_DATABASE_ROOT.child(NODE_GROUP).push().key.toString()
    val path = REF_DATABASE_ROOT.child(NODE_GROUP).child(keyGroup)
...
fun sendMessageToGroup(message: String, groupID: String, typeText: String, function: () -> Unit) {
    ...
    val RefMessages = "$NODE_GROUP/$groupID/$NODE_MESSAGES"
